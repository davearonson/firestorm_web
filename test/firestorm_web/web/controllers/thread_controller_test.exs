defmodule FirestormWeb.Web.ThreadControllerTest do
  ...
  @create_attrs %{title: "some title", body: "i ain't got no body"}
  ...
  def fixture(category, :thread) do
    attrs = @create_attrs |> Map.put(:category_id, category.id)
  ...
  test "creates thread and redirects to show when data is valid",
  ...
  test "does not create thread and renders errors when data is invalid",
       %{conn: conn, category: category} do
    conn = post conn, category_thread_path(conn, :create, category), thread: @invalid_attrs
    assert html_response(conn, 200) =~ "New Thread"
  end

  test "renders form for editing chosen thread",
       %{conn: conn, category: category} do
  ...
  test "updates chosen thread and redirects when data is valid",
       %{conn: conn, category: category} do
    thread = fixture(category, :thread)
  ...
  test "does not update chosen thread and renders errors when data is invalid",
       %{conn: conn, category: category} do
    thread = fixture(category, :thread)
